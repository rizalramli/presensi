            ...
            <div class="row">
                @if (!empty($is_absensi))
                    <div class="col-12">
                        <div class="card text-center">
                            <div class="card-body">
                                <h5 class="card-title">Anda sudah melakukan absen masuk pada pukul
                                    {{ date('H:i', strtotime($is_absensi->jam_masuk)) }}</h5>
                            </div>
                        </div>
                    </div>
                ...
                @else
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <a href="{{ route('absensi.masuk-absensi.index') }}" class="btn btn-sm btn-primary mb-3"> <i
        ...
        function submitLocation() {
            if (userMarker) {
                ...
                if (capturedImage.length === 0 || !capturedImage.attr("src")) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{{ route('absensi.masuk-absensi.store') }}",
                        data: {
                            latitude_masuk: userLocation.lat,
                            longitude_masuk: userLocation.lng,
                            status_lokasi_masuk: status_lokasi,
                            image: capturedImage
                        },
                        success: function(response) {
                            Swal.fire({
                                icon: 'success',
                                title: 'Sukses!',
                                text: 'Sukses melakukan absensi masuk',
                                willClose: () => {
